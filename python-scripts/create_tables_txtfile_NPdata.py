...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 30 15:41:02 2018

Takes the information from canonical NP database and creates all tables for the 
structure database which can be used in SQLite.
Command line: python3 create_tables_txtfile_NPdata.py Canonical_db_file_newest_version.txt
@author: stokm006
"""
...
def create_structure_table(all_info_list):
    ...
    for i, line in enumerate(all_info_list):
        if smile != line[2]:   
            structure_dict['NP_{}'.format(len(structure_dict)+1)] = \
            [line[0], line[1], line[2], line[4], line[5], line[6], line[7],\
            line[8], line[9], line[10], line[11]]
            structure_dict2['NP_{}'.format(len(structure_dict2)+1)] = \
    ...
    with open("/mnt/scratch/stokm006/create_official_NPdb/structure_data.txt", 'w') as db_file:
...
def create_structure_database_table(smile_list, all_info_dict):
    ...
    for line in my_list2:
        structure_database_dict['NP_SRC_{}'.format(len(structure_database_dict)+1)]\
        = line
    
    # create Structure_Database_table txt file  
    with open("/mnt/scratch/stokm006/create_official_NPdb/structure_database_data.txt", 'w') as db_file:
...
def create_database_table(structure_database_dict):
    ...
    with open("/mnt/scratch/stokm006/create_official_NPdb/database_data.txt", 'w') as db_file:
