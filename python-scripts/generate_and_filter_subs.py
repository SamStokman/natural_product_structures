...
structures and then filters the substructures on mol weight, presence of hetero
atoms, nr of rings in the substructure, abundancy and nr of the bonds which
are connected to the structure.

Command line: python3 generate_and_filter_subs.py simple_structures.txt n n
@author: stokm006
"""
...
def generate_substructures(input_file, create_files):
   ...
   # for line in all_lines[0:2]:
    ...
    if create_files == 'y':
        multiple_molecules = Draw.MolsToGridImage(structure_mol_list, molsPerRow=4,subImgSize=(200,150), legends = structure_smile_list)
        multiple_molecules.save('/mnt/scratch/stokm006/generate_substructures/filter/official/final_new_structures.png')       
     
    # generate all smile combinations for each atom and length
    for i, structure_info in enumerate(structure_combo_list):
        ...
        for substructure in all_subs_comb_list:
            ...
            if mol_sub != None and substructure not in sub_smile_list and substructure != '':
    ...
    if create_files == 'y':
        ...
        with open("test2_new3_substructures1.txt", 'w') as db_file:
        ...
        multiple_molecules.save('/mnt/scratch/stokm006/generate_substructures/filter/official/test3_new2_substructures.png')       
...
def filter5(structure_info, filter4_dict, draw_substructures):
                ...
                for smile in values:
                    if smile != '<NA>':
                        ...
                        if input_mol.HasSubstructMatch(sub_mol) == True:
                            ...
                            if min_nr_of_bonds <= 1:    # adjust this value
    ...
    with open("final_newtrash_simple_substructures.txt", 'w') as db_file:
    ...
    if draw_substructures == 'y':
        # To draw all_substructures       
        multiple_molecules = Draw.MolsToGridImage(draw_list, molsPerRow=5,subImgSize=(250,150), legends = draw_legend_list)
        multiple_molecules.save('/mnt/scratch/stokm006/generate_substructures/filter/official/final_trash_new_simple_substructures.png')       
