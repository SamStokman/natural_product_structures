...
# TO DRAW ALL INPUT STRUCTURES
  ...
  #  multiple_molecules.save('/mnt/scratch/stokm006/generate_substructures/filter/indo_names_structures.png')       
    ...
    for mol in structure_mol_list:
        ...
        whole_structure = Chem.MolToSmiles(mol)
        ...
        for smile in substructure_list_temp:
            ...
            if smile != whole_structure and smile not in substructure_list:
    ...
    for sub_struc in substructure_list:
        sm = Chem.MolFromSmarts(sub_struc)
        if sm != None:
...
#   
 ...
 #                   all_subs_dict[name] = [substruc]
  ...
  #  all_subs_dict_sorted = sorted(all_subs_dict)  # to order the structures in abscending order
  #  with open("wedtest_all_flavo_substructures.txt", 'w') as db_file:
...
#
#       TO DRAW ALL POSSIBLE SUBSTRUCTURES IN A PNG FILE
# 
   # draw_mol_list = []
   # legends_list = []
   # for subs in substructure_mol_list:
   #     subs = Chem.MolToSmiles(subs)
   #     legends_list += [subs]
   #     subs = Chem.MolFromSmiles(subs, sanitize=False)
   #     draw_mol_list += [subs]

 #   multiple_molecules = Draw.MolsToGridImage(draw_mol_list, molsPerRow=8,subImgSize=(150,150), legends = legends_list)
 #   multiple_molecules.save('/mnt/scratch/stokm006/generate_substructures/filter/wed_pyra_all_substructures.png')       
###############################################################################
  
    # Filter 1; Hetero atoms (N, O, Cl, F, Br, I, At, Zr & B)
    # And mol weigth, NOTE: hydrogen ignored, it is not possible to use Descriptors.ExactMolWt(m)
    ...
    for i in range(len(substructure_list)):
        ...
        if substructure_list[i][-1] == 'l': # for Cl at the end of the smile
            c_count += -1
        if substructure_list[i][-1] == 'r': # for Br at the end of the smile
            B_count += -1
   ...
   #     print ('mol weigth  ', mw)
    ...
    # Filter2: for abundancy and nr of rings
    substructure_mol_list = []
    ...
    # abundancy = percentage of structures where the substructure is present in
    abundancy_dict = {}
    ...
    for key, value in abundancy_dict.items():
        if value[1] >= 20 and value[1] <= 80:    # adjust these values
            key_mol = Chem.MolFromSmiles(key, sanitize=False) #was smarts
            nr_ring_matches = Chem.GetSSSR(key_mol)
            if nr_ring_matches >= 0:
                filter2_list += [key]
                filter2_mol_list += [key_mol]

    # filter3; the maximum number of bonds that connects the substructure to the structure
    filter3_list = []
    filter3_mol_list = []

    for structure_combo in structure_combo_list:
        ref_dict = {}
        empty_dict = {}
        input_mol = structure_combo[0]
        ...
        for sub_mol in filter2_mol_list:
            if input_mol.HasSubstructMatch(sub_mol) == True:
                this_dict =  empty_dict
                match = input_mol.GetSubstructMatch(sub_mol)#make it into matches!
                for i, num in enumerate(match):
                    if num in this_dict:
                        atom =  sub_mol.GetAtomWithIdx(i)
                        nr_of_neighbors = len([z.GetAtomicNum() for z in atom.GetNeighbors()])
                        this_dict[num] = nr_of_neighbors
                # determine the nr of bonds per atom in the substructure        
                difference_dict = {}
                for key, value in ref_dict.items():
                    difference = int(value) - int(this_dict[key])
                ...
                if nr_of_bonds <= 3:
                    if sub_mol not in filter3_mol_list:
                        filter3_mol_list += [sub_mol]
                        filter3_list += [Chem.MolToSmiles(sub_mol)]
                  
    print ('Final nr. of generated and filtered substructures:  ', len(filter3_list))
...
#    with open("wed_filtered_pyra_cycle_1_smile_substructures.txt", 'w') as db_file:
...
#    draw_mol_list = []
#    legends_list = []
#    for subs in filter3_mol_list:
#        subs = Chem.MolToSmiles(subs)
#        legends_list += [subs]
#        subs = Chem.MolFromSmiles((subs), sanitize=False)
#        draw_mol_list += [subs]
#
#    multiple_molecules = Draw.MolsToGridImage(draw_mol_list, molsPerRow=5,subImgSize=(150,150), legends = legends_list)
#    multiple_molecules.save('/mnt/scratch/stokm006/generate_substructures/filter/thr_examp_substructures.png')       
###############################################################################
